...
from reward_poisoned_drl.federated.runner import FederatedRunnerLogNotify
from reward_poisoned_drl.federated.server import FederatedServerLogNotify
from reward_poisoned_drl.federated.client.serial import SerialFederatedClientLogNotify
...
def build_and_train(run_ID=0, cuda_idx=None, n_parallel=2, serial_sampling=False):
    ...
    clients = [SerialFederatedClientLogNotify(AsaFactoryClean(Sampler=Sampler)) for _ in range(num_clean_clients)]
    clients += [SerialFederatedClientLogNotify(AsaFactoryMalicious(Sampler=Sampler)) for _ in range(num_malicious_clients)]

    client_per_itr = 2
    global_agent, global_sampler = get_agent_and_sampler(Sampler=Sampler, need_eval=True)
    server = FederatedServerLogNotify(global_agent, global_sampler, clients_per_itr=client_per_itr)

    runner = FederatedRunnerLogNotify(
        server,
        clients,
        n_itr=5e6,
        affinity=affinity,
        log_interval_itrs=5000
    )
